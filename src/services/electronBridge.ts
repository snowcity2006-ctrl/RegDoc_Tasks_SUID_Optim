...
import { INITIAL_SUID_TASKS } from '../data/initialSuidTasks';
...
const STORAGE_KEYS = {
  ...
  PROJECTS: 'docflow_projects',
  DOCUMENTS: 'docflow_documents',
  TASKS: 'docflow_tasks',
  SUID_TASKS: 'docflow_suid_tasks',
...
const INITIAL_DATA = {
  ...
  projects: [
    {
      id: 1,
      name: 'Обустройство Харасавэйского ГКМ',
      code: 'ХГКМ-0825',
      organizationIds: [1, 4],
      organizationNames: 'АО «НПО РусБИТех» (Astra Linux), ООО «Газпром Автоматизация»',
      gipEmployeeIds: [1, 4],
      gipEmployeeNames: 'Иванов Иван Иванович, Петров Сергей Николаевич',
      createdAt: '2026-08-10T10:00:00.000Z',
      updatedAt: '2026-09-02T12:00:00.000Z',
    },
    {
      id: 2,
      name: 'Модернизация защищенной сетевой инфраструктуры ЦОД',
      code: 'ЦОД-2026-АСТРА',
      organizationIds: [2, 5],
      organizationNames: 'ПАО «Ростелеком», ГК «Астра»',
      gipEmployeeIds: [2, 5],
      gipEmployeeNames: 'Смирнова Елена Александровна, Васильева Ольга Дмитриевна',
...
class WebMockDatabase implements ElectronAPI {
  ...
  private initDatabaseIfEmpty() {
    ...
    try {
      // 1. Типы документов (РД, ТТ, ПД)
      const rawTypes = localStorage.getItem(STORAGE_KEYS.DOC_TYPES);
      const currentTypes: DocumentType[] = rawTypes ? JSON.parse(rawTypes) : [];
      const requiredTypes = ['РД', 'ТТ', 'ПД'];
      let typesChanged = false;
      let nextTypeId = currentTypes.length > 0 ? Math.max(...currentTypes.map((t) => t.id)) + 1 : 1;
      for (const req of requiredTypes) {
        if (!currentTypes.some((t) => t.name.trim().toLowerCase() === req.toLowerCase())) {
          currentTypes.push({ id: nextTypeId++, name: req });
          typesChanged = true;
        }
      }
      if (typesChanged) {
        localStorage.setItem(STORAGE_KEYS.DOC_TYPES, JSON.stringify(currentTypes));
      }

      // 2. Проекты (4674, 825, 6342, 6570, 7288, 4680, 5675, 6513, 5677)
      const rawProjects = localStorage.getItem(STORAGE_KEYS.PROJECTS);
      const currentProjects: Project[] = rawProjects ? JSON.parse(rawProjects) : [];
      const suidProjectsList = [
        { code: '4674', name: 'Дообустройство сеноман-аптских залежей Бованенковского НГКМ' },
        { code: '825', name: 'Обустройство сеноман-апских залежей Харасавэйского ГКМ. Газопровод подключения Харасавэйского ГКМ' },
        { code: '6342', name: 'Реконструкция полигона твердых бытовых и производственных отходов Бованенковского НГКМ' },
        { code: '6570', name: 'Реконструкция и техническое перевооружение электромодуля №1 ЭСН на промбазе ГП-2 Бованенковского НГКМ (2 очередь, 1 этап)' },
        { code: '7288', name: 'Техническое перевооружение скважин Бованенковского НГКМ (замена НКТ)' },
        { code: '4680', name: 'Дообустройство Юбилейного НГКМ (нижнемеловые отложения)' },
        { code: '5675', name: 'Дообустройство сеноман-аптских залежей Харасавэйского ГКМ (1-й этап)' },
        { code: '6513', name: 'Техническое перевооружение ГПА ДКС (2 и 3 очередей) ГП-1, ГП-2 Бованенковского НГКМ (замена СПЧ)' },
        { code: '5677', name: 'Реконструкция ДКС Ямсовейского НГКМ (2-я очередь)' },
      ];
      let projectsChanged = false;
      let nextProjId = currentProjects.length > 0 ? Math.max(...currentProjects.map((p) => p.id)) + 1 : 1;
      for (const sp of suidProjectsList) {
        const exists = currentProjects.some((p) => p.code === sp.code || p.name.trim().toLowerCase() === sp.name.toLowerCase());
        if (!exists) {
          currentProjects.push({
            ...
            name: sp.name,
            organizationIds: [1],
            organizationNames: 'АО «НПО РусБИТех» (Astra Linux)',
            gipEmployeeIds: [],
            gipEmployeeNames: '',
      ...
      if (projectsChanged) {
        localStorage.setItem(STORAGE_KEYS.PROJECTS, JSON.stringify(currentProjects));
      }

      // 3. Подразделения
      const rawDepts = localStorage.getItem(STORAGE_KEYS.DEPARTMENTS);
      const currentDepts: Department[] = rawDepts ? JSON.parse(rawDepts) : [];
      const suidDepts = [
        'ИТЦ', 'ОГЭ', 'ЯЭГ', 'СКЗ', 'УАВР', 'ПрО', 'СПиПБ', 'ЯГПУ', 'ОПР',
        'ОН', 'ОРОФ', 'НИЛ', 'ОГМ', 'ПОпоДиПТГ', 'ГКиН', 'ООТ', 'УСКиС',
        'ОУИ', 'УС', 'ННГДУ', 'ПОА', 'УЭВП', 'ПОпоДГ', 'ОООС', 'СпецОтдел',
        'СД', 'СГМ', 'ПОМО', 'УЯЭГ', 'ПОпоДиПкТГ'
      ];
      let deptsChanged = false;
      let nextDeptId = currentDepts.length > 0 ? Math.max(...currentDepts.map((d) => d.id)) + 1 : 1;
      for (const dName of suidDepts) {
        if (!currentDepts.some((d) => d.shortName.toLowerCase() === dName.toLowerCase())) {
          currentDepts.push({
            id: nextDeptId++,
            name: `Подразделение ${dName}`,
            shortName: dName,
            organizationId: 1,
          });
          deptsChanged = true;
      ...
      // 4. Сотрудники (кураторы и авторы)
      const rawEmps = localStorage.getItem(STORAGE_KEYS.EMPLOYEES);
      const currentEmps: Employee[] = rawEmps ? JSON.parse(rawEmps) : [];
      const suidEmps = [
        { fullName: 'Бурлакин М.И.', position: 'Куратор от ОПР', departmentShortName: 'ОПР' },
        { fullName: 'Бычков И.Н.', position: 'Куратор от ОПР', departmentShortName: 'ОПР' },
        { fullName: 'Ковач О.Л.', position: 'Куратор от ОПР', departmentShortName: 'ОПР' },
        { fullName: 'Назарова С.А.', position: 'Куратор от ОПР', departmentShortName: 'ОПР' },
        { fullName: 'Демченко А.С.', position: 'Куратор от ОПР', departmentShortName: 'ОПР' },
        { fullName: 'Куликов П.В.', position: 'Куратор от ОПР', departmentShortName: 'ОПР' },
        { fullName: 'Антонова Светлана Викторовна', position: 'Ведущий инженер', departmentShortName: 'ОПР' },
        { fullName: 'Куц Вячеслав Александрович', position: 'Главный специалист', departmentShortName: 'ОПР' },
        { fullName: 'Мавлютов Айрат Рашитович', position: 'Ведущий инженер', departmentShortName: 'ИТЦ' },
        { fullName: 'Туполенко Сергей Петрович', position: 'Ведущий специалист', departmentShortName: 'ОГЭ' },
        { fullName: 'Урюпин Дмитрий Николаевич', position: 'Главный специалист', departmentShortName: 'ЯЭГ' },
      ];
      let empsChanged = false;
      let nextEmpId = currentEmps.length > 0 ? Math.max(...currentEmps.map((e) => e.id)) + 1 : 1;
      for (const se of suidEmps) {
        if (!currentEmps.some((e) => e.fullName.trim().toLowerCase() === se.fullName.toLowerCase())) {
          currentEmps.push({
            id: nextEmpId++,
            fullName: se.fullName,
            position: se.position,
            departmentShortName: se.departmentShortName,
            organizationId: 1,
          });
          empsChanged = true;
        }
  ...
  async getProjects(): Promise<Project[]> {
    ...
    return list.map((proj) => {
      const orgNames = (proj.organizationIds || [])
        .map((id) => orgs.find((o) => o.id === id)?.name)
        .filter(Boolean)
        .join(', ');

      const gipNames = (proj.gipEmployeeIds || [])
        .map((id) => emps.find((e) => e.id === id)?.fullName)
        .filter(Boolean)
        .join(', ');
      ...
      return {
        ...
        gipEmployeeNames: gipNames,
      };
    });
  }

  async saveProject(project: Omit<Project, 'id' | 'createdAt' | 'updatedAt'> & { id?: number }): Promise<Project> {
    ...
    const cleanCode = project.code ? project.code.trim() : '';

    // Проверка на дубликат наименования проекта (без учета регистра)
    const isDuplicate = list.some(
      (item) => item.id !== project.id && item.name.trim().toLowerCase() === cleanName.toLowerCase()
    );
    if (isDuplicate) {
      throw new Error(`Проект с наименованием «${cleanName}» уже существует в справочнике`);
    }

    const orgNames = (project.organizationIds || [])
      .map((id) => orgs.find((o) => o.id === id)?.name)
      .filter(Boolean)
      .join(', ');

    const gipNames = (project.gipEmployeeIds || [])
      .map((id) => emps.find((e) => e.id === id)?.fullName)
      .filter(Boolean)
      .join(', ');
    ...
    if (project.id) {
      const idx = list.findIndex((i) => i.id === project.id);
      ...
      saved = {
        ...
        organizationNames: orgNames,
        gipEmployeeIds: project.gipEmployeeIds || [],
        gipEmployeeNames: gipNames,
        updatedAt: now,
      };
      list[idx] = saved;
      await this.addLog('info', 'db', `Обновлен проект: "${saved.name}" (ID: ${saved.id})`);
    } else {
      const newId = list.length > 0 ? Math.max(...list.map((i) => i.id)) + 1 : 1;
      saved = {
        id: newId,
      ...
      list.push(saved);
      await this.addLog('info', 'db', `Добавлен проект: "${saved.name}" (${saved.code || 'без кода'})`);
    }
  ...
  async deleteProject(id: number): Promise<{ success: boolean }> {
    ...
    this.touchUpdateTime();
    await this.addLog('warn', 'db', `Удален проект ID: ${id}`);
    return { success: true };
  }
  ...
  // --- Работа в СУИД ---
  async getSuidTasks(): Promise<SuidTaskRecord[]> {
    const raw = localStorage.getItem(STORAGE_KEYS.SUID_TASKS);
    const tasks: SuidTaskRecord[] = raw ? JSON.parse(raw) : [];
    return tasks.map((t, idx) => ({
      ...t,
      idx: t.idx ?? idx + 1,
      participatingDepartments: Array.isArray(t.participatingDepartments) ? t.participatingDepartments : [],
      branchReports: Array.isArray(t.branchReports) ? t.branchReports : [],
      curatorEmployeeIds: Array.isArray(t.curatorEmployeeIds) ? t.curatorEmployeeIds : [],
    }));
  }

  async getSuidTaskById(id: number): Promise<SuidTaskRecord | null> {
    const list = await this.getSuidTasks();
    const found = list.find((t) => t.id === id);
    return found || null;
  }

  async saveSuidTask(task: Omit<SuidTaskRecord, 'id' | 'createdAt' | 'updatedAt'> & { id?: number }): Promise<SuidTaskRecord> {
    const list = await this.getSuidTasks();
    const now = new Date().toISOString();
    let saved: SuidTaskRecord;

    // Автоподстановка связанных наименований если не переданы
    let docTypeName = task.docTypeName || '';
    if (task.docTypeId && !docTypeName) {
      const types = await this.getDocumentTypes();
      const t = types.find((item) => item.id === task.docTypeId);
      if (t) docTypeName = t.name;
    }

    let projectCode = task.projectCode || '';
    let projectName = task.projectName || '';
    if (task.projectId && (!projectCode || !projectName)) {
      const projects = await this.getProjects();
      const p = projects.find((item) => item.id === task.projectId);
      if (p) {
        if (!projectCode) projectCode = p.code || '';
        if (!projectName) projectName = p.name || '';
      }
    }

    let curatorNames = task.curatorNames || '';
    if (task.curatorEmployeeIds && task.curatorEmployeeIds.length > 0 && !curatorNames) {
      const employees = await this.getEmployees();
      const names = task.curatorEmployeeIds
        .map((cid) => employees.find((e) => e.id === cid)?.fullName)
        .filter(Boolean);
      if (names.length > 0) {
        curatorNames = names.join(', ');
      }
    }

    // Расчет дней просрочки
    let delayDays = task.delayDays ?? 0;
    if (task.plannedEndDate) {
      if (task.actualEndDate) {
        const pParts = task.plannedEndDate.split('-').map(Number);
        const aParts = task.actualEndDate.split('-').map(Number);
    ...
    if (task.id) {
      const idx = list.findIndex((t) => t.id === task.id);
      if (idx === -1) throw new Error('Запись СУИД не найдена');
      saved = {
        ...list[idx],
        ...task,
        docTypeName,
      ...
      list[idx] = saved;
      await this.addLog('info', 'db', `Обновлена запись СУИД №${saved.id} (${saved.taskName})`);
    } else {
      const newId = list.length > 0 ? Math.max(...list.map((t) => t.id)) + 1 : 1;
      ...
      list.push(saved);
      await this.addLog('info', 'db', `Создана запись СУИД №${saved.id} (${saved.taskName})`);
    }

    localStorage.setItem(STORAGE_KEYS.SUID_TASKS, JSON.stringify(list));
    this.touchUpdateTime();
    return saved;
  }

  async saveSuidTasks(taskList: Array<Omit<SuidTaskRecord, 'id' | 'createdAt' | 'updatedAt'>>): Promise<SuidTaskRecord[]> {
    const list = await this.getSuidTasks();
    const now = new Date().toISOString();
    let maxId = list.length > 0 ? Math.max(...list.map((t) => t.id)) : 0;
    const created: SuidTaskRecord[] = [];

    for (const t of taskList) {
      maxId += 1;
      const record: SuidTaskRecord = {
        ...t,
        id: maxId,
        idx: t.idx ?? maxId,
        createdAt: now,
    ...
    localStorage.setItem(STORAGE_KEYS.SUID_TASKS, JSON.stringify(list));
    this.touchUpdateTime();
    await this.addLog('info', 'db', `Импортировано/сохранено записей СУИД: ${created.length}`);
    return created;
  }

  async deleteSuidTask(id: number): Promise<{ success: boolean }> {
    const list = await this.getSuidTasks();
    const filtered = list.filter((t) => t.id !== id);
    localStorage.setItem(STORAGE_KEYS.SUID_TASKS, JSON.stringify(filtered));
    this.touchUpdateTime();
    await this.addLog('warn', 'db', `Удалена запись СУИД ID: ${id}`);
    return { success: true };
  }
...
export const electronBridge: ElectronAPI = new Proxy({} as ElectronAPI, {
  get(_target, prop: keyof ElectronAPI) {
    ...
    if (prop === 'saveSuidTasks' && typeof window !== 'undefined' && window.electronAPI && typeof window.electronAPI.saveSuidTask === 'function') {
      return async (taskList: Array<Omit<SuidTaskRecord, 'id' | 'createdAt' | 'updatedAt'>>) => {
        const createdList: SuidTaskRecord[] = [];
        for (const t of taskList) {
          const res = await window.electronAPI.saveSuidTask(t);
          createdList.push(res);
        }
        return createdList;
      };
