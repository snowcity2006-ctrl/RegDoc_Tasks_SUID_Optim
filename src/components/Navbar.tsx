...
export interface NavbarProps {
  ...
  onOpenBackups?: () => void;
  onOpenLogs?: () => void;
  activeTab?: 'documents' | 'directories' | 'tasks' | 'suid';
  onSelectTab?: (tab: 'documents' | 'directories' | 'tasks' | 'suid') => void;
  ...
  return (
    <header className="sticky top-0 z-30 bg-[#171A21] border-b border-[#2D3139] transition-colors shadow-xs w-full">
      <div className="w-full px-4 sm:px-6">
        ...
        {/* Мобильная строка синхронизации */}
        <div className="flex sm:hidden items-center justify-end py-1.5 border-t border-[#2D3139] text-[10px] font-mono text-gray-500">
          Синхронизация: {displayUpdateTime}
        </div>
      </div>
    </header>
  );
