...
export const DocumentFormModal: React.FC<DocumentFormModalProps> = ({
  ...
  const toggleRecipient = useCallback((orgId: number) => {
    setRecipientIds((prev) =>
      prev.includes(orgId) ? prev.filter((id) => id !== orgId) : [...prev, orgId]
    );
  }, []);

  const removeRecipient = useCallback((orgId: number, e?: React.MouseEvent) => {
  ...
  const handleClearRecipients = useCallback(() => {
    setRecipientIds([]);
  }, []);
  ...
  const handleSenderOrgChange = useCallback((newOrgId: number | '') => {
    ...
    if (newOrgId) {
      setSenderDepartmentId((prevDeptId) => {
        if (prevDeptId) {
          const dept = departments.find((d) => d.id === prevDeptId);
      ...
      setSenderEmployeeId((prevEmpId) => {
        if (prevEmpId) {
          const emp = employees.find((e) => e.id === prevEmpId);
      ...
      setSignatoryEmployeeId((prevSigId) => {
        if (prevSigId) {
          const emp = employees.find((e) => e.id === prevSigId);
          if (emp && emp.organizationId !== newOrgId) {
            return '';
          }
        }
  ...
  const handleSignatoryEmployeeChange = useCallback((newEmpId: number | '') => {
    setSignatoryEmployeeId(newEmpId);
    if (newEmpId) {
      const emp = employees.find((e) => e.id === newEmpId);
      if (emp) {
        setSenderId((prevOrgId) => {
          if (!prevOrgId || prevOrgId !== emp.organizationId) {
  ...
  const handleSenderDeptChange = useCallback((newDeptId: number | '') => {
    setSenderDepartmentId(newDeptId);
    if (newDeptId) {
      const dept = departments.find((d) => d.id === newDeptId);
      if (dept) {
        setSenderId((prevOrgId) => {
          if (!prevOrgId || prevOrgId !== dept.organizationId) {
            return dept.organizationId;
          }
          return prevOrgId;
        });
      }
  ...
  const handleSenderEmployeeChange = useCallback((newEmpId: number | '') => {
    ...
    if (newEmpId) {
      ...
      if (emp) {
        ...
        setSenderDepartmentId((prevDeptId) => {
          if (!prevDeptId && emp.departmentShortName) {
