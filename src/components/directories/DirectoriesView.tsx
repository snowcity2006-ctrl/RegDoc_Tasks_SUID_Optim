...
interface DirectoriesViewProps {
  ...
  onSaveProject: (project: Omit<Project, 'id' | 'createdAt' | 'updatedAt'> & { id?: number }) => Promise<void>;
  onDeleteProject: (id: number) => Promise<void>;
}

type DirectoryTab = 'orgs' | 'depts' | 'emps' | 'docTypes' | 'directions' | 'projects';
...
export const DirectoriesView: React.FC<DirectoriesViewProps> = ({
  ...
  const [projectSort, setProjectSort] = useState<{ field: string; asc: boolean }>({ field: 'name', asc: true });
  ...
  const defaultProjectWidths: Record<string, number> = { idx: 65, name: 320, code: 160, organizations: 260, gip: 240, actions: 100 };
  ...
  const [projectWidths, setProjectWidths] = useState<Record<string, number>>(() => {
    try {
      const saved = localStorage.getItem('sed_dir_project_widths');
      return saved ? JSON.parse(saved) : defaultProjectWidths;
    } catch {
      return defaultProjectWidths;
    }
  });
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    ...
    if (tab === 'orgs') {
      ...
      setProjectWidths((prev) => {
        const next = { ...prev, [colKey]: newWidth };
        latestWidthsRef.current.projects = next;
        return next;
      });
    }
  }, []);
  ...
  const handleMouseUp = useCallback(() => {
    ...
    if (currentResizing) {
      try {
        ...
        } else if (currentResizing.tab === 'projects') {
          localStorage.setItem('sed_dir_project_widths', JSON.stringify(latestWidthsRef.current.projects));
        }
      } catch {}
    }
  }, [handleMouseMove]);
  ...
  const sortedProjects = useMemo(() => {
    const filtered = (projects || []).filter((p) => {
      ...
      return (
        ...
        (p.organizationNames && p.organizationNames.toLowerCase().includes(q)) ||
        (p.gipEmployeeNames && p.gipEmployeeNames.toLowerCase().includes(q)) ||
        String(p.id).includes(q)
      );
    });

    return [...filtered].sort((a, b) => {
      let valA: any = (a as any)[projectSort.field] || '';
      let valB: any = (b as any)[projectSort.field] || '';

      if (projectSort.field === 'idx') {
        valA = a.id;
        valB = b.id;
      } else if (projectSort.field === 'organizations') {
        valA = a.organizationNames || '';
        valB = b.organizationNames || '';
      } else if (projectSort.field === 'gip') {
        valA = a.gipEmployeeNames || '';
        valB = b.gipEmployeeNames || '';
      }

      if (typeof valA === 'string') {
        const cmp = valA.localeCompare(valB, 'ru', { numeric: true, sensitivity: 'base' });
        return projectSort.asc ? cmp : -cmp;
      }
      return projectSort.asc ? (valA > valB ? 1 : -1) : (valA < valB ? 1 : -1);
    });
  }, [projects, q, projectSort.field, projectSort.asc]);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-[#171A21] rounded-2xl p-4 sm:p-5 border border-[#2D3139] shadow-xs space-y-4">
        ...
        <div className="flex flex-wrap gap-2 border-t border-[#2D3139] pt-3">
          ...
          >
            ...
            <span>Проекты</span>
            <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-mono ${
              activeTab === 'projects' ? 'bg-blue-700 text-white' : 'bg-[#1F222B] text-gray-400'
            }`}>
              {(projects || []).length}
            </span>
          </button>
      ...
      {activeTab === 'projects' && (
        <div className="bg-[#171A21] rounded-2xl border border-[#2D3139] shadow-xs overflow-hidden flex flex-col">
          ...
          {/* Таблица: прокрутка колесиком мыши, фиксированная шапка, видимые границы колонок и строк */}
          <div className="overflow-x-auto overflow-y-auto max-h-[620px] custom-scrollbar">
            <table className="w-full text-left border-collapse text-xs select-none table-fixed border-b border-[#2D3139]">
              <colgroup>
                <col style={{ width: `${projectWidths.idx}px` }} />
                <col style={{ width: `${projectWidths.name}px` }} />
                <col style={{ width: `${projectWidths.code}px` }} />
                <col style={{ width: `${projectWidths.organizations}px` }} />
                <col style={{ width: `${projectWidths.gip}px` }} />
                <col style={{ width: `${projectWidths.actions}px` }} />
              </colgroup>
              <thead className="sticky top-0 z-10 bg-[#0F1115] shadow-xs">
                <tr className="border-b border-[#2D3139] text-gray-400 font-semibold uppercase tracking-wider bg-[#0F1115]">
                  {/* 1.1. № */}
                  <th
                    style={{ width: `${projectWidths.idx}px`, minWidth: 50 }}
                    className="py-3 px-4 relative group overflow-hidden border-r border-[#2D3139] bg-[#0F1115]"
                  >
                    <div
                      onClick={() => handleSortToggle(projectSort, setProjectSort, 'idx')}
                      className="flex items-center justify-between cursor-pointer min-w-0 pr-1.5"
                    >
                      <span className="truncate block" title="№">№</span>
                      {renderSortIcon('idx', projectSort.field, projectSort.asc)}
                    </div>
                    <div
                      onMouseDown={(e) => startResizing('projects', 'idx', projectWidths.idx, e)}
                      className="absolute right-0 top-0 bottom-0 w-1.5 cursor-col-resize hover:bg-blue-500 z-10"
                    />
                  </th>
                  ...
                  <th
                    style={{ width: `${projectWidths.name}px`, minWidth: 150 }}
                    className="py-3 px-4 relative group overflow-hidden border-r border-[#2D3139] bg-[#0F1115]"
                  >
                    <div
                      onClick={() => handleSortToggle(projectSort, setProjectSort, 'name')}
                      className="flex items-center justify-between cursor-pointer min-w-0 pr-1.5"
                    >
                      <span className="truncate block" title="Название проекта">Название проекта *</span>
                      {renderSortIcon('name', projectSort.field, projectSort.asc)}
                    </div>
                    <div
                      onMouseDown={(e) => startResizing('projects', 'name', projectWidths.name, e)}
                      className="absolute right-0 top-0 bottom-0 w-1.5 cursor-col-resize hover:bg-blue-500 z-10"
                    />
                  </th>
                  ...
                  {/* 1.3. Код проекта */}
                  <th
                    style={{ width: `${projectWidths.code}px`, minWidth: 100 }}
                    className="py-3 px-4 relative group overflow-hidden border-r border-[#2D3139] bg-[#0F1115]"
                  >
                    <div
                      onClick={() => handleSortToggle(projectSort, setProjectSort, 'code')}
                      className="flex items-center justify-between cursor-pointer min-w-0 pr-1.5"
                    >
                      <span className="truncate block" title="Код проекта">Код проекта</span>
                      {renderSortIcon('code', projectSort.field, projectSort.asc)}
                    </div>
                    <div
                      onMouseDown={(e) => startResizing('projects', 'code', projectWidths.code, e)}
                      className="absolute right-0 top-0 bottom-0 w-1.5 cursor-col-resize hover:bg-blue-500 z-10"
                    />
                  </th>

                  {/* 1.4. Проектная организация */}
                  <th
                    style={{ width: `${projectWidths.organizations}px`, minWidth: 150 }}
                    className="py-3 px-4 relative group overflow-hidden border-r border-[#2D3139] bg-[#0F1115]"
                  >
                    <div
                      onClick={() => handleSortToggle(projectSort, setProjectSort, 'organizations')}
                      className="flex items-center justify-between cursor-pointer min-w-0 pr-1.5"
                    >
                      <span className="truncate block" title="Проектная организация">Проектная организация</span>
                      {renderSortIcon('organizations', projectSort.field, projectSort.asc)}
                    </div>
                    <div
                      onMouseDown={(e) => startResizing('projects', 'organizations', projectWidths.organizations, e)}
                      className="absolute right-0 top-0 bottom-0 w-1.5 cursor-col-resize hover:bg-blue-500 z-10"
                    />
                  </th>
                  ...
                  {/* 1.5. ГИП */}
                  <th
                    style={{ width: `${projectWidths.gip}px`, minWidth: 150 }}
                    className="py-3 px-4 relative group overflow-hidden border-r border-[#2D3139] bg-[#0F1115]"
                  >
                    <div
                      onClick={() => handleSortToggle(projectSort, setProjectSort, 'gip')}
                      className="flex items-center justify-between cursor-pointer min-w-0 pr-1.5"
                    >
                      <span className="truncate block" title="ГИП">ГИП</span>
                      {renderSortIcon('gip', projectSort.field, projectSort.asc)}
                    </div>
                    <div
                      onMouseDown={(e) => startResizing('projects', 'gip', projectWidths.gip, e)}
                      className="absolute right-0 top-0 bottom-0 w-1.5 cursor-col-resize hover:bg-blue-500 z-10"
                    />
                  </th>
                  ...
                  {/* 1.6. Действия */}
                  <th
                    style={{ width: `${projectWidths.actions}px`, minWidth: 70 }}
                    className="py-3 px-4 text-right overflow-hidden bg-[#0F1115]"
                  >
                    <span className="truncate block" title="Действия">Действия</span>
                  </th>
                </tr>
              ...
              <tbody className="divide-y divide-[#2D3139] text-[#E0E0E0] font-medium">
                {sortedProjects.map((project, index) => (
                  <tr key={project.id} className="hover:bg-[#1F222B]/60 transition-colors border-b border-[#2D3139]">
                    {/* 1.1. № */}
                    <td className="py-3 px-4 font-mono text-gray-500 break-words overflow-hidden border-r border-[#2D3139] text-center">
                      {index + 1}
                    </td>
                    ...
                    {/* 1.4. Проектная организация (множественный выбор) */}
                    <td className="py-3 px-4 text-gray-300 break-words whitespace-normal leading-relaxed overflow-hidden border-r border-[#2D3139]">
                      {project.organizationNames ? (
                        <div className="flex flex-wrap gap-1.5">
                          {project.organizationNames.split(', ').map((orgName, i) => (
                            <span
                              key={i}
                    ...
                    {/* 1.5. ГИП (множественный выбор) */}
                    <td className="py-3 px-4 text-gray-300 break-words whitespace-normal leading-relaxed overflow-hidden border-r border-[#2D3139]">
                      {project.gipEmployeeNames ? (
                        <div className="flex flex-wrap gap-1.5">
                          {project.gipEmployeeNames.split(', ').map((gipName, i) => (
                            <span
                              key={i}
                      ...
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <button
                          onClick={() =>
                            confirmDelete(
                              'Удаление проекта',
                              `Вы действительно хотите удалить проект «${project.name}»? Действие нельзя отменить.`,
                ...
                {sortedProjects.length === 0 && (
                  <tr>
                    <td colSpan={6} className="py-8 text-center text-gray-500 border-b border-[#2D3139]">
                      Проекты не найдены
                    </td>
                  </tr>
                )}
