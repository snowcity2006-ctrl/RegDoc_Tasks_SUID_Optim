...
export default function App() {
  ...
  // Основная навигация: 'documents' | 'directories' | 'tasks' | 'suid'
  const [activeTab, setActiveTab] = useState<'documents' | 'directories' | 'tasks' | 'suid'>('documents');

  // Данные базы
  const [documents, setDocuments] = useState<DocumentRecord[]>([]);
  const [tasks, setTasks] = useState<TaskRecord[]>([]);
  const [suidTasks, setSuidTasks] = useState<SuidTaskRecord[]>([]);
  ...
  const loadAllData = useCallback(async () => {
    try {
      const [status, orgs, depts, emps, types, dirs, docs, tsks, projs, suids] = await Promise.all([
        ...
        electronBridge.getProjects(),
        electronBridge.getSuidTasks(),
      ]);
  ...
  // CRUD: Проекты
  const handleSaveProject = useCallback(async (projData: Omit<Project, 'id' | 'createdAt' | 'updatedAt'> & { id?: number }) => {
    const saved = await electronBridge.saveProject(projData);
    showNotification(projData.id ? `Проект «${saved.name}» обновлен` : `Проект «${saved.name}» добавлен`);
    await loadAllData();
  }, [loadAllData]);
  ...
  // CRUD: СУИД (Работа в СУИД)
  const handleSaveSuidTask = useCallback(async (suidData: Omit<SuidTaskRecord, 'id' | 'createdAt' | 'updatedAt'> & { id?: number }) => {
    const saved = await electronBridge.saveSuidTask(suidData);
    showNotification(suidData.id ? `Запись СУИД «${saved.taskName}» обновлена` : `Запись СУИД «${saved.taskName}» добавлена`);
    await loadAllData();
  }, [loadAllData]);

  const handleDeleteSuidTask = useCallback(async (id: number) => {
    await electronBridge.deleteSuidTask(id);
    showNotification('Запись СУИД удалена');
    await loadAllData();
  }, [loadAllData]);
  ...
  return (
    <div className="min-h-screen bg-[#0F1115] text-[#E0E0E0] flex flex-col font-sans transition-colors duration-200 antialiased">
      ...
      <main className="flex-1 w-full px-3 sm:px-4 py-3 sm:py-4 space-y-4">
        ...
        {/* Раздел: Модуль СУИД (Система Управления Инженерными Данными) */}
        {activeTab === 'suid' && (
          <SuidView
            tasks={suidTasks}
            onSaveTask={handleSaveSuidTask}
            onDeleteTask={handleDeleteSuidTask}
            documentTypes={documentTypes}
            projects={projects}
