...
class SQLiteDatabaseManager {
  ...
  public syncAllRelatedRecords(): void {
    ...
    try {
      ...
      try {
        ...
        if (hasProjectsTable) {
          const orgRows = this.db.prepare('SELECT id, name FROM organizations').all() as Array<{ id: number; name: string }>;
          const orgMap = new Map(orgRows.map((o) => [o.id, o.name]));

          const empRows = this.db.prepare('SELECT id, full_name FROM employees').all() as Array<{ id: number; full_name: string }>;
          const empMap = new Map(empRows.map((e) => [e.id, e.full_name]));

          const allProjects = this.db.prepare('SELECT id, organization_ids, organization_names, gip_employee_ids, gip_employee_names FROM projects').all() as Array<{
            id: number;
            organization_ids: string | null;
            organization_names: string | null;
            gip_employee_ids: string | null;
            gip_employee_names: string | null;
          }>;

          const updateProjStmt = this.db.prepare('UPDATE projects SET organization_names = ?, gip_employee_names = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?');

          for (const proj of allProjects) {
            let orgIds: number[] = [];
            try {
              if (proj.organization_ids) orgIds = JSON.parse(proj.organization_ids);
            } catch {}
            const expectedOrgNames = orgIds.map((id) => orgMap.get(id)).filter(Boolean).join(', ');

            let gipIds: number[] = [];
            try {
              if (proj.gip_employee_ids) gipIds = JSON.parse(proj.gip_employee_ids);
            } catch {}
            const expectedGipNames = gipIds.map((id) => empMap.get(id)).filter(Boolean).join(', ');

            if (expectedOrgNames !== (proj.organization_names || '') || expectedGipNames !== (proj.gip_employee_names || '')) {
              updateProjStmt.run(expectedOrgNames, expectedGipNames, proj.id);
            }
          }
        }
      } catch {}
  ...
  private async ensureColumnsExist() {
    ...
    try {
      ...
      // Проверка существования таблицы projects
      await this.runWithRetry(() => {
        this.db.exec(`
          CREATE TABLE IF NOT EXISTS projects (
            ...
            gip_employee_ids TEXT,
            gip_employee_names TEXT,
            created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
            updated_at DATETIME DEFAULT CURRENT_TIMESTAMP
          );
          ...
          CREATE TABLE IF NOT EXISTS suid_tasks (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            idx INTEGER,
            receipt_date TEXT,
            ...
            project_name TEXT,
            notes TEXT,
            participating_depts TEXT,
            branch_reports TEXT,
            is_report_not_required INTEGER DEFAULT 0,
            curator_ids TEXT,
            curator_names TEXT,
            created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
            updated_at DATETIME DEFAULT CURRENT_TIMESTAMP
          );
          CREATE INDEX IF NOT EXISTS idx_suid_dates ON suid_tasks (receipt_date, planned_end_date, actual_end_date);
          CREATE INDEX IF NOT EXISTS idx_suid_project ON suid_tasks (project_id, project_code);
          CREATE INDEX IF NOT EXISTS idx_suid_doc_type ON suid_tasks (doc_type_id);
        `);
      });
    } catch (e: any) {
  ...
  private populateFullSchemaAndDefaults(targetDb: any) {
    ...
    try {
      ...
      // Первичное наполнение проектами, если projects пуста
      try {
        const projCountRow = targetDb.prepare('SELECT count(*) as c FROM projects').get() as { c: number } | undefined;
        if (!projCountRow || projCountRow.c === 0) {
          const insertProj = targetDb.prepare(`
            INSERT INTO projects (name, code, organization_ids, organization_names, gip_employee_ids, gip_employee_names)
            VALUES (?, ?, ?, ?, ?, ?)
          `);
          insertProj.run(
            'Обустройство Харасавэйского ГКМ',
            'ХГКМ-0825',
            JSON.stringify([1, 4]),
            'АО «НПО РусБИТех» (Astra Linux), ООО «Газпром Автоматизация»',
            JSON.stringify([1, 4]),
            'Иванов Иван Иванович, Петров Сергей Николаевич'
          );
          insertProj.run(
            'Модернизация защищенной сетевой инфраструктуры ЦОД',
            'ЦОД-2026-АСТРА',
            JSON.stringify([2, 5]),
            'ПАО «Ростелеком», ГК «Астра»',
            JSON.stringify([2, 5]),
            'Смирнова Елена Александровна, Васильева Ольга Дмитриевна'
          );
        }
      } catch {}
  ...
  public getProjects(): Project[] {
    ...
    const rows = this.db.prepare(`
      SELECT id, name, code,
             organization_ids as organizationIdsRaw,
             organization_names as organizationNames,
             gip_employee_ids as gipEmployeeIdsRaw,
             gip_employee_names as gipEmployeeNames,
    ...
    return rows.map((r) => {
      ...
      if (r.organizationIdsRaw) {
        try {
          const parsed = JSON.parse(r.organizationIdsRaw);
          if (Array.isArray(parsed)) organizationIds = parsed;
        } catch {}
      }
      let gipEmployeeIds: number[] = [];
      if (r.gipEmployeeIdsRaw) {
        try {
          const parsed = JSON.parse(r.gipEmployeeIdsRaw);
          if (Array.isArray(parsed)) gipEmployeeIds = parsed;
        } catch {}
      }
      ...
      return {
        ...
        gipEmployeeIds,
        gipEmployeeNames: r.gipEmployeeNames || '',
  ...
  public async saveProject(project: Omit<Project, 'id' | 'createdAt' | 'updatedAt'> & { id?: number }): Promise<Project> {
    return await this.runWriteTransaction(() => {
      ...
      // Проверка на дубликат наименования
      const dupCheck = project.id
        ? this.db.prepare('SELECT id FROM projects WHERE LOWER(name) = LOWER(?) AND id != ?').get(cleanName, project.id)
        : this.db.prepare('SELECT id FROM projects WHERE LOWER(name) = LOWER(?)').get(cleanName);
      if (dupCheck) {
        throw new Error(`Проект с наименованием «${cleanName}» уже существует в справочнике`);
      }

      const orgIds = project.organizationIds || [];
      const gipIds = project.gipEmployeeIds || [];
      const orgIdsJson = orgIds.length > 0 ? JSON.stringify(orgIds) : null;
      const gipIdsJson = gipIds.length > 0 ? JSON.stringify(gipIds) : null;

      // Вычисляем текстовые наименования
      let orgNames = '';
      if (orgIds.length > 0) {
        const placeholders = orgIds.map(() => '?').join(',');
        const orgRows = this.db.prepare(`SELECT name FROM organizations WHERE id IN (${placeholders})`).all(...orgIds) as Array<{ name: string }>;
        orgNames = orgRows.map((o) => o.name).join(', ');
      }

      let gipNames = '';
      if (gipIds.length > 0) {
        const placeholders = gipIds.map(() => '?').join(',');
        const empRows = this.db.prepare(`SELECT full_name FROM employees WHERE id IN (${placeholders})`).all(...gipIds) as Array<{ full_name: string }>;
        gipNames = empRows.map((e) => e.full_name).join(', ');
      }
      ...
      if (project.id) {
        this.db.prepare(`
          UPDATE projects
          SET name = ?, code = ?, organization_ids = ?, organization_names = ?,
              gip_employee_ids = ?, gip_employee_names = ?, updated_at = CURRENT_TIMESTAMP
          WHERE id = ?
        `).run(
          cleanName,
      ...
      if (!row) throw new Error('Не удалось прочитать сохраненный проект');

      let savedOrgIds: number[] = [];
      if (row.organizationIdsRaw) {
        try {
          const parsed = JSON.parse(row.organizationIdsRaw);
          if (Array.isArray(parsed)) savedOrgIds = parsed;
        } catch {}
      }
      let savedGipIds: number[] = [];
      if (row.gipEmployeeIdsRaw) {
        try {
          const parsed = JSON.parse(row.gipEmployeeIdsRaw);
          if (Array.isArray(parsed)) savedGipIds = parsed;
        } catch {}
      }
      ...
      return {
        ...
        name: row.name,
        code: row.code || '',
        organizationIds: savedOrgIds,
        organizationNames: row.organizationNames || '',
        gipEmployeeIds: savedGipIds,
        gipEmployeeNames: row.gipEmployeeNames || '',
  ...
  public async deleteProject(id: number): Promise<{ success: boolean }> {
    return await this.runWriteTransaction(() => {
  ...
  // --- CRUD Работа в СУИД ---
  public getSuidTasks(): SuidTaskRecord[] {
    if (!this.db || !this.db.open) return [];
    try {
      const rows = this.db.prepare(`
        SELECT id, idx,
               ...
               notes,
               participating_depts as participatingDeptsRaw,
               branch_reports as branchReportsRaw,
               is_report_not_required as isReportNotRequired,
               curator_ids as curatorIdsRaw,
               curator_names as curatorNames,
               created_at as createdAt,
               updated_at as updatedAt
        FROM suid_tasks
        ORDER BY id ASC
      ...
      return rows.map((r: any) => {
        let participatingDepartments: any[] = [];
        if (r.participatingDeptsRaw) {
          try {
            const parsed = JSON.parse(r.participatingDeptsRaw);
            if (Array.isArray(parsed)) participatingDepartments = parsed;
          } catch {}
        }

        let branchReports: any[] = [];
        if (r.branchReportsRaw) {
          try {
            const parsed = JSON.parse(r.branchReportsRaw);
            if (Array.isArray(parsed)) branchReports = parsed;
          } catch {}
        }

        let curatorEmployeeIds: number[] = [];
        if (r.curatorIdsRaw) {
          try {
            const parsed = JSON.parse(r.curatorIdsRaw);
            if (Array.isArray(parsed)) curatorEmployeeIds = parsed;
          } catch {}
        }
        ...
        return {
          ...
          plannedEndDate: r.plannedEndDate || '',
          actualEndDate: r.actualEndDate || '',
          delayDays: r.delayDays !== null && r.delayDays !== undefined ? Number(r.delayDays) : 0,
          taskName: r.taskName || '',
          taskDescription: r.taskDescription || '',
          suidId: r.suidId || '',
          authorName: r.authorName || '',
          docTypeId: r.docTypeId || undefined,
          ...
          branchReports,
          isReportNotRequired: Boolean(r.isReportNotRequired),
          curatorEmployeeIds,
          curatorNames: r.curatorNames || '',
          createdAt: r.createdAt,
          updatedAt: r.updatedAt,
        };
      });
  ...
  public getSuidTaskById(id: number): SuidTaskRecord | null {
    if (!this.db || !this.db.open) return null;
    const r = this.db.prepare(`
      SELECT id, idx,
             receipt_date as receiptDate,
  ...
  public async saveSuidTask(task: Omit<SuidTaskRecord, 'id' | 'createdAt' | 'updatedAt'> & { id?: number }): Promise<SuidTaskRecord> {
    return await this.runWriteTransaction(() => {
      let targetId: number;
      const partDeptsJson = JSON.stringify(task.participatingDepartments || []);
      const branchReportsJson = JSON.stringify(task.branchReports || []);
      const curatorIdsJson = JSON.stringify(task.curatorEmployeeIds || []);
      const isRepNotReq = task.isReportNotRequired ? 1 : 0;

      if (task.id) {
        this.db.prepare(`
          UPDATE suid_tasks SET
            idx = ?, receipt_date = ?, planned_end_date = ?, actual_end_date = ?,
            delay_days = ?, task_name = ?, task_description = ?, suid_id = ?,
            author_name = ?, doc_type_id = ?, doc_type_name = ?,
            project_id = ?, project_code = ?, project_name = ?,
            notes = ?, participating_depts = ?, branch_reports = ?,
            is_report_not_required = ?, curator_ids = ?, curator_names = ?,
            updated_at = CURRENT_TIMESTAMP
          WHERE id = ?
        `).run(
          task.idx ?? task.id, task.receiptDate || '', task.plannedEndDate || '', task.actualEndDate || '',
          task.delayDays ?? 0, task.taskName, task.taskDescription || '', task.suidId || '',
          task.authorName || '', task.docTypeId || null, task.docTypeName || '',
          task.projectId || null, task.projectCode || '', task.projectName || '',
          task.notes || '', partDeptsJson, branchReportsJson,
          isRepNotReq, curatorIdsJson, task.curatorNames || '',
          task.id
        );
        targetId = task.id;
      } else {
        const info = this.db.prepare(`
          INSERT INTO suid_tasks (
            idx, receipt_date, planned_end_date, actual_end_date,
            delay_days, task_name, task_description, suid_id,
            author_name, doc_type_id, doc_type_name,
            project_id, project_code, project_name,
        ...
        `).run(
          task.idx ?? 1, task.receiptDate || '', task.plannedEndDate || '', task.actualEndDate || '',
          task.delayDays ?? 0, task.taskName, task.taskDescription || '', task.suidId || '',
          task.authorName || '', task.docTypeId || null, task.docTypeName || '',
          task.projectId || null, task.projectCode || '', task.projectName || '',
          task.notes || '', partDeptsJson, branchReportsJson,
      ...
      const saved = this.getSuidTaskById(targetId);
      if (!saved) throw new Error('Не удалось прочитать сохраненную запись СУИД');
      return saved;
    });
  }

  public async saveSuidTasks(taskList: Array<Omit<SuidTaskRecord, 'id' | 'createdAt' | 'updatedAt'>>): Promise<SuidTaskRecord[]> {
    if (!this.db || !this.db.open) throw new Error('База данных не инициализирована');
    if (!taskList || taskList.length === 0) return [];
  ...
  public async deleteSuidTask(id: number): Promise<{ success: boolean }> {
    return await this.runWriteTransaction(() => {
      this.db.prepare('DELETE FROM suid_tasks WHERE id = ?').run(id);
      return { success: true };
    });
  }
