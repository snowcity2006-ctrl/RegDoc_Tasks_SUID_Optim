...
if (process.platform === 'linux') {
  ...
  // Оптимизация многопоточного программного рендеринга для исключения задержек набора текста в Astra Linux:
  // По умолчанию Chromium в режиме программного рендеринга выделяет всего 1 поток CPU на растеризацию,
  // что приводило к фризам ввода символов в полях форм. Выделение потоков устраняет просадку FPS.
  app.commandLine.appendSwitch('num-raster-threads', '4');
  app.commandLine.appendSwitch('enable-zero-copy');

  // Если рабочая станция Astra Linux оснащена видеокартой со стабильными драйверами,
  // пользователь или администратор может включить аппаратное GPU-ускорение переменной ELECTRON_ENABLE_GPU=1
  const forceGpu = process.env.ELECTRON_ENABLE_GPU === '1' || process.env.ASTRAGPU === '1';
  if (forceGpu) {
    app.commandLine.appendSwitch('ignore-gpu-blocklist');
    app.commandLine.appendSwitch('enable-gpu-rasterization');
  } else {
    // Безопасный режим: предотвращает падение Chromium GPU-процесса в Astra Linux без графических драйверов
    try {
...
function setupIpcHandlers() {
  ...
  ipcMain.handle('project:getAll', async () => dbManager.getProjects());
  ipcMain.handle('project:save', async (_e, project) => dbManager.saveProject(project));
  ipcMain.handle('project:delete', async (_e, id) => dbManager.deleteProject(id));
  ...
  // --- Работа в СУИД ---
  ipcMain.handle('suid:getAll', async () => dbManager.getSuidTasks());
  ipcMain.handle('suid:getById', async (_e, id) => dbManager.getSuidTaskById(id));
  ipcMain.handle('suid:save', async (_e, task) => dbManager.saveSuidTask(task));
  ipcMain.handle('suid:saveTasks', async (_e, tasks) => dbManager.saveSuidTasks(tasks));
  ipcMain.handle('suid:delete', async (_e, id) => dbManager.deleteSuidTask(id));
